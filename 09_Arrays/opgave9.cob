      ...
      * Array til kontooplysninger
       01  KONTO-ARRAY.
           05 KONTO-RECORD OCCURS 20 TIMES.
               10 KUNDE-ID        PIC X(10).
               10 KONTO-ID        PIC X(10).
               10 KONTO-TYPE      PIC X(20).
               10 BALANCE         PIC Z(6)9V99.
               10 VALUTA-KD       PIC X(3).
      ...
      * Populate KONTO-RECORD array fra INPUT-FILE-KONTOOPL
       POPULATE-KONTO-RECORDS.
       OPEN INPUT INPUT-FILE-KONTOOPL.
       SET EOF-NOT-REACHED TO TRUE
       PERFORM VARYING IX FROM 1 BY 1
        UNTIL EOF-REACHED OR IX > 20
           READ INPUT-FILE-KONTOOPL INTO KONTO-INPUT-RECORD
               AT END
                   SET EOF-REACHED TO TRUE
               NOT AT END
                   MOVE KONTO-INPUT-RECORD TO KONTO-RECORD(IX)
           END-READ
       END-PERFORM
       ...
       PERFORM VARYING IX FROM 1 BY 1
        UNTIL IX > 20
           IF KUNDE-ID IN KONTO-RECORD(IX) = KUNDE-ID IN KUNDEOPL
               IF IS-FIRST-ENTRY
                   MOVE SPACES TO OUTPUT-TEXT
                   STRING 
               "-------------------------------------------------"
               ...
               MOVE SPACES TO OUTPUT-TEXT
               STRING
                   KONTO-ID IN KONTO-RECORD(IX) " | "
                   KONTO-TYPE IN KONTO-RECORD(IX) " | "
                   BALANCE IN KONTO-RECORD(IX) " " 
                   VALUTA-KD IN KONTO-RECORD(IX)
                   INTO OUTPUT-TEXT
               WRITE OUTPUT-RECORD
           END-IF
       END-PERFORM.
